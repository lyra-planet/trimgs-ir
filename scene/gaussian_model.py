...
class FlattenGaussianModel(GaussianModel):
    ...
    def densify_and_scale_split(
        ...
        scale_mask = (
            ...
            > self.percent_dense * scene_extent * scale_factor
        )
        if max_screen_size:
            scale_mask = torch.logical_or(
                scale_mask, self.max_radii2D > max_screen_size
            )
        ...
        prune_filter = torch.logical_or(
            prune_filter, (self.get_opacity < min_opacity).squeeze()
        )
        self.prune_points(prune_filter)
