...
from gaussian_renderer import GaussianModel,FlattenGaussianModel, render
...
def eval_brdf(data_root: str, scene: Scene, model_path: str, name: str) -> None:
    ...
    lpips_fn = LPIPS(net="vgg").cuda()
    print(f"begin to evaluate BRDF for {name}")
    ...
    print(f"three_channel_ratio: {three_channel_ratio}")
    for idx, (mask, albedo_map, albedo_gt) in enumerate(tqdm(zip(masks, albedo_maps, albedo_gts))):
        ...
        print(f"albedo psnr: {get_psnr(albedo_gt, albedo_map).mean().double()}")
